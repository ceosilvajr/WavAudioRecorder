...
/**
 * Created by ceosi_000 on 5/15/2014.
 */
...
public class WavAudioRecorder {
     ...
     */
    public WavAudioRecorder(boolean uncompressed, int audioSource, int sampleRate, int channelConfig,
                            int audioFormat) {
        try {
            ...
            if (rUncompressed) { // RECORDING_UNCOMPRESSED
                ...
                if (bufferSize < AudioRecord.getMinBufferSize(sampleRate, channelConfig, audioFormat)) { // Check to make sure buffer size is not smaller than the smallest allowed one
                    ...
                    Log.w(WavAudioRecorder.class.getName(), "Increasing buffer size to " + Integer.toString(bufferSize));
                }
